...
# Default save location for temporary outputs (relative to process launch directory)
# This keeps all saved artifacts under ./tmp by default when running the script.
SAVE_TMP_DIR = os.path.abspath(os.path.join(os.getcwd(), "SaveOutput"))
...
async def simple_crawl():
    ...
    async with AsyncWebCrawler(config=browser_config) as crawler:
        result = await crawler.arun(
            ...
            url="https://docs.unity3d.com/Packages/com.unity.shadergraph@17.4/manual/Node-Library.html", config=crawler_config
        )
...
def _sanitize_provider_for_filename(provider: str) -> str:
    if not provider:
        return "dom"
    # provider strings may include slashes and colons like 'ollama/phi3:3.8b' or 'gpt-4.1'
    token = provider
    # take the last segment after slash
    if "/" in token:
        token = token.split("/")[-1]
    # replace colon and other non-filename chars with dash
    token = re.sub(r"[^A-Za-z0-9._-]", "-", token)
    return token


def save_to_json(result, out_path: str = None, provider: str = None):
    """Save a crawl/extraction result to JSON in a tmp folder.

    Tries to parse result.extracted_content as JSON, otherwise stores it as a raw string.
    Also includes url, success and basic markdown fields when available.
    """
    tmp_dir = SAVE_TMP_DIR
    os.makedirs(tmp_dir, exist_ok=True)

    if out_path is None:
        token = _sanitize_provider_for_filename(provider)
        out_path = os.path.join(tmp_dir, f"unity_shadergraph_{token}.json")
    ...
    if isinstance(extracted, str):
        try:
            payload["extracted"] = json.loads(extracted)
        except Exception:
            payload["extracted_raw"] = extracted
    else:
        payload["extracted"] = extracted

    # markdown is saved separately by save_to_markdown()

    # Write file
    with open(out_path, "w", encoding="utf-8") as f:
        json.dump(payload, f, indent=2, ensure_ascii=False)

    print(f"Wrote LLM extraction to {out_path}")


def save_to_markdown(result, out_path: str = None):
    """Save the generated markdown (raw and fit) to a markdown file in tmp folder.

    If out_path is not provided, writes to docs/tmp/unity_shadergraph_llm.md
    """
    tmp_dir = SAVE_TMP_DIR
    os.makedirs(tmp_dir, exist_ok=True)

    if out_path is None:
        # Attempt to pull provider from result if available
        provider = getattr(result, "provider", None)
        token = _sanitize_provider_for_filename(provider)
        out_path = os.path.join(tmp_dir, f"unity_shadergraph_{token}.md")

    md_text = None
    if getattr(result, "markdown", None):
        try:
            md_text = getattr(result.markdown, "raw_markdown", None) or getattr(result.markdown, "fit_markdown", None)
        except Exception:
            try:
    ...
    if md_text is None:
        md_text = ""  # write an empty file when markdown not available
...
async def extract_structured_data_using_llm(
    ...
    # Allow local Ollama providers (e.g. "ollama/phi3:3.8b") to run without an API token.
    if api_token is None and not (
        isinstance(provider, str) and provider.startswith("ollama")
    ):
    ...
    # Save extraction result to JSON file and also save markdown separately
    save_to_json(result, provider=provider)
    # Attach provider to result so save_to_markdown can pick it up if needed
    try:
        setattr(result, "provider", provider)
    except Exception:
        pass
